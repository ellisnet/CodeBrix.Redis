...
/// <summary>
/// Diagnostics reported by the analyzers and generators shipped inside the CodeBrix.Redis package.
/// </summary>
/// <remarks>
/// <para>
/// The <c>SER</c> identifier space is shared with the <c>[Experimental]</c> API gates in
/// <c>CodeBrix.Redis.Respite.Experiments</c>, which own <c>SER0xx</c> and mean something quite different
/// ("this API is preview"). Everything reported by <em>this</em> assembly lives in <c>SER3xx</c>, split as:
/// </para>
/// <list type="bullet">
...
internal static class Diagnostics
{
    ...
    /// <para>
    /// The host is upstream's, deliberately, and the same goes for the two <c>SyncOverAsync</c> links below.
    /// These rules are ported unchanged from StackExchange.Redis and describe exactly the same code shapes;
    /// upstream's pages are the only published prose that explains them, and pointing a consumer at a page
    /// that does not exist would be worse than pointing them at someone else's. The IDs are upstream's too
    /// (see the class remarks), so the pages match one for one.
    /// </para>
    /// </remarks>
