...
namespace CodeBrix.Redis.Build; //was previously: StackExchange.Redis.Build;
...
public sealed class QueuedResultAnalyzer : DiagnosticAnalyzer
{
    ...
    {
        private KnownSymbols(List<IMethodSymbol> blockingHelpers, INamedTypeSymbol? redisAsync, INamedTypeSymbol? batch, INamedTypeSymbol? transactionAsync, INamedTypeSymbol? transaction, INamedTypeSymbol? commandFlags, INamedTypeSymbol? task, int? fireAndForgetValue, bool reportBlockingHelper)
        {
            ...
            ReportBlockingHelper = reportBlockingHelper;
        }
        ...
        /// <summary>
        /// Whether SER308 applies to the compilation being analysed - true for everyone except the library
        /// that declares the blocking helpers, and this repository's own test assembly.
        /// </summary>
        /// <remarks>
        /// DELIBERATE MODIFICATION OF THE PORTED ANALYZER, and the only behavioural change made to it.
        /// SER308 tells a CONSUMER not to reach for <c>Wait</c>/<c>WaitAll</c>/<c>TryWait</c>; inside
        /// CodeBrix.Redis itself those helpers ARE the synchronous surface, so its own calls are the rule's
        /// subject matter rather than a mistake - the <c>IRedisAsync</c> forwarders every decorator must
        /// implement, and the sync-over-async in <c>RedisBase</c> and <c>CursorEnumerable</c> that no
        /// rewrite can remove without changing behaviour. Upstream marks all of them intentional with
        /// <c>#pragma warning disable SER308</c>; this repository carries no pragmas, so the exemption
        /// lives here instead, gated on the assembly name exactly as <c>AutoDatabaseGenerator</c> is. A
        /// consumer's compilation has some other assembly name, so what a consumer sees from SER308 is
        /// byte-for-byte upstream's behaviour.
        /// <para>
        /// The same exemption covers <c>CodeBrix.Redis.Tests</c>, the ported upstream suite, for the same
        /// reason: those tests exist to exercise the SYNCHRONOUS API, so awaiting instead of blocking would
        /// change what they test. Upstream carries 17 <c>#pragma warning disable SER308</c> lines there,
        /// which this repository does not replicate.
        /// </para>
        /// </remarks>
        private bool ReportBlockingHelper { get; }

        /// <summary>The assembly whose own source is exempt from SER308; see <see cref="ReportBlockingHelper"/>.</summary>
        private const string OwningAssembly = "CodeBrix.Redis";

        /// <summary>This repository's own test assembly, exempt on the same grounds; see <see cref="ReportBlockingHelper"/>.</summary>
        private const string OwningTestAssembly = "CodeBrix.Redis.Tests";
        ...
        public static KnownSymbols? TryCreate(Compilation compilation)
        {
            ...
            var redisAsync = compilation.GetTypeByMetadataName("CodeBrix.Redis.IRedisAsync");
            var batch = compilation.GetTypeByMetadataName("CodeBrix.Redis.IBatch");
            var transactionAsync = compilation.GetTypeByMetadataName("CodeBrix.Redis.ITransactionAsync");
            if (redisAsync is null && batch is null && transactionAsync is null) return null;

            var commandFlags = compilation.GetTypeByMetadataName("CodeBrix.Redis.CommandFlags");
            ...
            foreach (var declaring in new[] { redisAsync, compilation.GetTypeByMetadataName("CodeBrix.Redis.IConnectionMultiplexer") })
            {
            ...
            return new KnownSymbols(
                ...
                batch,
                transactionAsync,
                compilation.GetTypeByMetadataName("CodeBrix.Redis.ITransaction"),
                commandFlags,
                compilation.GetTypeByMetadataName("System.Threading.Tasks.Task"),
                fireAndForget,
                reportBlockingHelper: compilation.AssemblyName is not (OwningAssembly or OwningTestAssembly));
        }
        ...
        {
            if (!ReportBlockingHelper) return false; // see the remarks on ReportBlockingHelper
