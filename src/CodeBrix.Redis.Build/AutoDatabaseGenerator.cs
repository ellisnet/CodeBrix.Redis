...
using MethodInfo = (string Name, string ReturnType, CodeBrix.Redis.Build.BasicArray<(string Name, string Type, Microsoft.CodeAnalysis.RefKind RefKind, bool IsParams, bool IsOptional, bool HasDefault, string? Default)> Parameters, CodeBrix.Redis.Build.BasicArray<string> TypeArgs);
using InterfaceInfo = (string Name, string Namespace, CodeBrix.Redis.Build.AutoDatabaseGenerator.KnownInterfaces KnownType, CodeBrix.Redis.Build.BasicArray<(string Name, string ReturnType, CodeBrix.Redis.Build.BasicArray<(string Name, string Type, Microsoft.CodeAnalysis.RefKind RefKind, bool IsParams, bool IsOptional, bool HasDefault, string? Default)> Parameters, CodeBrix.Redis.Build.BasicArray<string> TypeArgs)> Methods);
using ClassInfo = (string Name, string Namespace, CodeBrix.Redis.Build.AutoDatabaseGenerator.KnownInterfaces Interfaces, bool IsMutator);
...
public class AutoDatabaseGenerator : IIncrementalGenerator
{
    ...
    /// </summary>
    /// <remarks>
    /// This is repo-internal machinery, but it now ships as an analyzer inside the CodeBrix.Redis package
    ...
    /// <c>CodeBrix.Redis</c> declaration - so short-circuit on the assembly name first, and skip even the
    ...
    private static void Generate(SourceProductionContext ctx, ImmutableArray<InterfaceInfo> interfaces, ImmutableArray<ClassInfo> classes)
    {
        ...
        foreach (var cls in classes.Distinct())
        {
            ...
            const string CommandFlagsType = "CodeBrix.Redis.CommandFlags";
            const string RedisKeyType = "CodeBrix.Redis.RedisKey";
            const string RedisChannelType = "CodeBrix.Redis.RedisChannel";
            ...
            const string StreamPositionType = "CodeBrix.Redis.StreamPosition";
            ...
            void AppendTupleTypes()
            {
                for (int i = 0; i < tupleDefs.Count; i++)
                {
                    ...
                    if (cls.IsMutator)
                    {
                        writer.Append(" : global::CodeBrix.Redis.IMappableRedisArgs");
                        firstBase = false;
                    }

                    if (flagsArg >= 0)
                    {
                        writer.Append(firstBase ? " : " : ", ").Append("global::CodeBrix.Redis.IFlaggedRedisArgs");
                    }
                    writer.NewLine().Append("{").Indent();
                    for (int p = 0; p < parameters.Length; p++)
                    {
                    ...
                    if (flagsArg >= 0)
                    {
                        ...
                        writer.NewLine().Append(cls.IsMutator ? "readonly " : "")
                            .Append("global::CodeBrix.Redis.CommandFlags global::CodeBrix.Redis.IFlaggedRedisArgs.Flags => Arg")
                    ...
                    writer.NewLine().Append("public void Map(global::CodeBrix.Redis.IRedisArgsMutator mutator)")
                ...
                if (cls.IsMutator && allReturns.Count is not 0)
                {
                    ...
                    foreach (var retType in ordered)
                    {
                        writer.NewLine().Append(firstIface ? ": " : ", ")
                            .Append("global::CodeBrix.Redis.IRedisArgsResult<").Append(retType).Append(">");
                        firstIface = false;
                    }
                    ...
                    foreach (var retType in ordered)
                    {
                        writer.NewLine().NewLine().Append(retType).Append(' ')
                            .Append("global::CodeBrix.Redis.IRedisArgsResult<")
                            .Append(retType)
                            .Append(">.UnMap(global::CodeBrix.Redis.IRedisArgsMutator mutator, ")
