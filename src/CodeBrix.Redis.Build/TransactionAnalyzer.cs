...
public sealed class TransactionAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        context.RegisterCompilationStartAction(static ctx =>
        {
            ...
            // gated on the assembly name exactly as QueuedResultAnalyzer gates SER308 and as
            // AutoDatabaseGenerator gates itself. SER301 suggests replacing a WATCH transaction with a
            // newer single atomic command; tests/CodeBrix.Redis.Tests exists to exercise the transaction
            // surface itself, so taking that suggestion would delete what those tests test. Upstream marks
            // its 2 sites intentional with "#pragma warning disable SER301", which this repository does not
            // carry. Only SER301, and only that assembly: src/CodeBrix.Redis is NOT exempt, and a
            // consumer's compilation has some other assembly name, so a consumer sees upstream's behaviour.
            var reportNewerAtomicOperation = ctx.Compilation.AssemblyName is not OwningTestAssembly;
            ctx.RegisterOperationBlockAction(blockCtx => Analyze(blockCtx, known, declaredMinVersion, reportNewerAtomicOperation));
        });
    }

    /// <summary>This repository's own test assembly, exempt from SER301; see the comment in <see cref="Initialize"/>.</summary>
    ...
    private sealed class KnownSymbols
    {
        ...
        public static KnownSymbols? TryCreate(Compilation compilation)
        {
            ...
            if (compilation.GetTypeByMetadataName("CodeBrix.Redis.Condition") is not { } condition) return null;

            var transaction = compilation.GetTypeByMetadataName("CodeBrix.Redis.ITransaction");
            ...
            return new KnownSymbols(condition, transaction, transactionAsync, compilation.GetTypeByMetadataName("CodeBrix.Redis.CommandFlags"));
        }
    ...
    private static void Analyze(OperationBlockAnalysisContext context, KnownSymbols known, ServerVersion declaredMinVersion, bool reportNewerAtomicOperation)
    {
        foreach (var block in context.OperationBlocks)
        {
            ...
            foreach (var pair in usages)
            {
                ...
                // see the comment in Initialize: SER301 alone, in this repository's own test assembly alone
                if (found.Rule == Rule.NewerAtomicOperation && !reportNewerAtomicOperation) continue;
