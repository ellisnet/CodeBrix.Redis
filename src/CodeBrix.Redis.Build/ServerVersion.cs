...
internal readonly struct ServerVersion
{
    ...
    public static ServerVersion FromOptions(AnalyzerOptions? options)
    {
        if (options is not null)
        {
            ...
            // the MSBuild <RedisMinServerVersion> property, surfaced by the CompilerVisibleProperty declared
            // in the build/ props the package ships; the build_property. prefix is how MSBuild properties
            // arrive here. Both spellings are upstream's and are kept: they name redis, not the upstream
            // library, so a project migrating off StackExchange.Redis keeps its existing configuration.
