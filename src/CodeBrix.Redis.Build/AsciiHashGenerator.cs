...
public class AsciiHashGenerator : IIncrementalGenerator
{
    ...
    /// through an alias.
    /// </remarks>
    private const string AsciiHashAttributeName = "CodeBrix.Redis.Respite.AsciiHashAttribute";
        ...
        ImmutableArray<(string Namespace, string ParentType, string Name, int Count, int MaxChars, int MaxBytes)> enums)
    {
        ...
        sb.AppendLine("using CodeBrix.Redis;");
        ...
        foreach (var grp in enums.GroupBy(l => (l.Namespace, l.ParentType)))
        {
            ...
            foreach (var method in grp)
            {
                ...
                if (alwaysCaseSensitive)
                {
                    if (twoPart)
                    {
                        writer.NewLine().Append("global::CodeBrix.Redis.Respite.AsciiHash.HashCS(").Append(method.From.Name).Append(", out var cs0, out var cs1);");
                    }
                    else
                    {
                        writer.NewLine().Append("var cs0 = global::CodeBrix.Redis.Respite.AsciiHash.HashCS(").Append(method.From.Name).Append(");");
                    }
                }
                else
                {
                    if (twoPart)
                    {
                        writer.NewLine().Append("global::CodeBrix.Redis.Respite.AsciiHash.Hash(").Append(method.From.Name)
                ...
                void Write(bool caseSensitive)
                {
                                 ...
                                 .ThenBy(x => x.ParseText))
                    {
                        ...
                        if (len > 2 * AsciiHash.MaxBytesHashed)
                        {
                            ...
                            line.Append("global::CodeBrix.Redis.Respite.AsciiHash.")
        ...
        foreach (var grp in types.GroupBy(l => (l.Namespace, l.ParentType)))
        {
            ...
            foreach (var literal in grp)
            {
                ...
                else
                {
                    ...
                    writer.NewLine().Append(
                        "public static bool IsCI(ReadOnlySpan<byte> value, long uc) => uc == HashUC && global::CodeBrix.Redis.Respite.AsciiHash.SequenceEqualsCI(value, U8);");
                }
