...
internal sealed class PhysicalBridge : IDisposable
{
    ...
    // Upstream declares this field 'volatile' and silences the resulting CS0420 at ChangeState/
    // OnHeartbeat with a #pragma. This repository fixes it at source instead: the 'volatile'
    // modifier is dropped and every plain read goes through Volatile.Read, which is the SAME
    // acquire barrier volatile gave those reads. Nothing is weakened - the two mutating sites
    // already use Interlocked (a full fence), and the 'ref State' alias in OnHeartbeat was
    // ALREADY non-volatile under upstream's declaration: that is precisely what CS0420 says
    // ("a reference to a volatile field will not be treated as volatile"), so its reads keep
    ...
    public bool IsConnected => Volatile.Read(ref state) == (int)State.ConnectedEstablished;

    public bool IsConnecting => Volatile.Read(ref state) == (int)State.ConnectedEstablishing || Volatile.Read(ref state) == (int)State.Connecting;
    ...
    internal State ConnectionState => (State)Volatile.Read(ref state);
    ...
    // Upstream calls this from inside the library and silences the resulting CS0618 with pragmas at each
    // call site. Here the body lives in the non-obsolete internal core below and this member forwards to
    // it, so the library's own callers name the core instead. The [Obsolete] member, its signature, its
    // accessibility and its message are exactly upstream's, so a consumer sees no difference.
    public WriteResult TryWriteSync(Message message, bool isReplica) => TryWriteSyncCore(message, isReplica);

    internal WriteResult TryWriteSyncCore(Message message, bool isReplica)
    {
        ...
        var result = WriteMessageTakingWriteLockSyncCore(physical, message);
    ...
    internal void KeepAlive(bool forceRun = false)
    {
        ...
        if (msg != null)
        {
            ...
            var result = TryWriteSyncCore(msg, ServerEndPoint.IsReplica);
    ...
    {
        var tmp = physical;
        if (tmp != null && Volatile.Read(ref state) != (int)State.ConnectedEstablished)
        {
    ...
    internal bool TryEnqueue(List<Message> messages, bool isReplica)
    {
        ...
        foreach (var message in messages)
        {
            ...
            WriteMessageTakingWriteLockSyncCore(physical, message);
            LogNonPreferred(message.Flags, isReplica);
        }
        return true;
    }
    ...
    internal WriteResult WriteMessageTakingWriteLockSyncCore(PhysicalConnection physical, Message message)
    {
        ...
        if (Interlocked.CompareExchange(ref _backlogProcessorIsRunning, 1, 0) == 0)
        {
            ...
            try
            {
                ...
                var thread = new Thread(s => ((PhysicalBridge)s!).ProcessBacklog())
                {
                    ...
                    Name = "CodeBrix.Redis Backlog", // help anyone looking at thread-dumps
                };
    ...
    {
        if (Volatile.Read(ref state) == (int)State.Disconnected)
        {
            try
            {
