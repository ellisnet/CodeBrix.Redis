...
internal sealed partial class RedisServer : RedisBase, IServer
{
    ...
    public void ReplicaOf(EndPoint master, CommandFlags flags = CommandFlags.None)
    {
        ...
        if (GetTiebreakerRemovalMessage() is Message tieBreakerRemoval)
        {
            ...
            server.GetBridge(tieBreakerRemoval)?.TryWriteSyncCore(tieBreakerRemoval, server.IsReplica);
        }
        ...
        if (GetConfigChangeMessage() is Message configChangeMessage)
        {
            ...
            server.GetBridge(configChangeMessage)?.TryWriteSyncCore(configChangeMessage, server.IsReplica);
        }
    }
    ...
    // SaveType.ForegroundSave is [Obsolete]. Upstream names it in the two switch arms below and silences the
    // resulting CS0618 with a pragma; an obsolete ENUM MEMBER cannot be forwarded the way an obsolete method
    // can, so the arms match this non-obsolete constant instead. It is written relative to the member declared
    // immediately before it rather than as a magic number, so it stays tied to the enum's own declaration.
    private const SaveType ForegroundSaveValue = SaveType.BackgroundSave + 1;
    ...
    private static Message GetSaveMessage(SaveType type, CommandFlags flags = CommandFlags.None) => type switch
    {
        ...
        ForegroundSaveValue => Message.Create(-1, flags, RedisCommand.SAVE),
    ...
    private static ResultProcessor<bool> GetSaveResultProcessor(SaveType type) => type switch
    {
        ...
        ForegroundSaveValue => ResultProcessor.DemandOK,
        _ => throw new ArgumentOutOfRangeException(nameof(type)),
    };
