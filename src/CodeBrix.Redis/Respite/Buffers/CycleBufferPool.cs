...
/// <summary>
/// A <see cref="MemoryPool{T}"/> that can size a new page against the data already leased, so that a
/// buffer which keeps growing gets progressively larger pages instead of many small ones.
/// </summary>
/// <typeparam name="T">The element type of the buffers being pooled.</typeparam>
...
public abstract class CycleBufferPool<T> : MemoryPool<T>
{
    ...
    /// </summary>
    /// <param name="existing">The data already leased, used to choose the size of the new buffer.</param>
    /// <returns>The leased buffer.</returns>
