...
public partial struct CycleBuffer
{
    ...
    /// </summary>
    /// <param name="pool">The pool to lease pages from; <see cref="MemoryPool{T}.Shared"/> is used when omitted.</param>
    /// <param name="callback">An optional callback to notify when a page has been filled.</param>
    /// <returns>The new buffer; this is a mutable value that should be held in a field, not passed around.</returns>
    ...
    /// <summary>
    /// The pool that this buffer leases its pages from.
    /// </summary>
    ...
    /// <summary>
    /// Gets all the committed data as a single span, when it happens to occupy a single page.
    /// </summary>
    /// <param name="span">The committed data; empty when this buffer has never been written to.</param>
    /// <returns><c>True</c> if the committed data occupies a single page, otherwise <c>False</c>;
    /// when <c>False</c>, use <see cref="GetAllCommitted"/> instead.</returns>
    ...
    /// <summary>
    /// Indicates whether there is no committed data awaiting consumption.
    /// </summary>
    ...
    /// Marks committed data as fully consumed; it will no longer appear in later calls to <see cref="GetAllCommitted"/>.
    /// </summary>
    /// <param name="count">The number of bytes to discard from the front of the committed data.</param>
    ...
    /// <summary>
    /// Gets the total number of committed bytes awaiting consumption.
    /// </summary>
    /// <returns>The committed length, in bytes.</returns>
    ...
    /// <summary>
    /// Gets the first page of committed data as a span, when it is large enough to be worth consuming.
    /// </summary>
    /// <param name="minBytes">The minimum length to accept; <see cref="GetAnything"/> means "any
    /// non-empty buffer" and <see cref="GetFullPagesOnly"/> means "any full page".</param>
    /// <param name="span">The committed data from the first page.</param>
    /// <returns><c>True</c> if a span of at least the requested size was available, otherwise <c>False</c>.</returns>
