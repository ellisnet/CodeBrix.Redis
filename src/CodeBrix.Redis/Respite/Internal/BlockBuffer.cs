...
internal abstract partial class BlockBufferSerializer
{
    internal sealed class BlockBuffer : MemoryManager<byte>
    {
...
#if DEBUG
            _parent = parent;
            _leakSentinel = new LeakSentinel(parent);
        ...
        private readonly LeakSentinel _leakSentinel;
#endif
...
#if DEBUG
                GC.SuppressFinalize(_leakSentinel); // only have a finalizer in debug
        ...
        //
        // Upstream puts this finalizer on BlockBuffer itself, behind a "#pragma warning disable CA2015"
        // and that comment. This repository suppresses no diagnostics, so the DEBUG-only leak
        // notification lives on a separate finalizable object held by the buffer: it becomes
        // unreachable at exactly the same moment the buffer does, so the behaviour is identical, and
        // CA2015's actual concern - a MemoryManager<T> finalizer freeing memory a Span<T> still points
        // at - cannot arise at all.
        private sealed class LeakSentinel(BlockBufferSerializer parent)
        {
            ~LeakSentinel()
            {
                parent.DebugBufferLeaked();
