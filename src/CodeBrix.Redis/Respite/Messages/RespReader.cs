...
public ref partial struct RespReader
{
    ...
    // (declared here rather than in RespReader.Span.cs, where they are used - see the note in that file)
    private ref byte _bufferRoot;
    private int _bufferLength;

#if DEBUG
    // (declared here rather than in RespReader.Debug.cs, where it belongs: an auto-property has a
    // backing field, and every instance field of a partial struct must be in the same declaration)
    ...
    /// <summary>
    /// The number of protocol bytes still available to this reader, across all remaining segments.
    /// </summary>
    ...
    /// <summary>
    /// Projects the element a reader is positioned on into a value.
    /// </summary>
    /// <typeparam name="T">The type of data being projected.</typeparam>
    /// <param name="value">The reader, positioned on the element to project.</param>
    /// <returns>The projected value.</returns>
    public delegate T Projection<out T>(ref RespReader value);

    /// <summary>
    /// Projects the element a reader is positioned on into a value, with additional caller state.
    /// </summary>
    /// <typeparam name="TState">Additional state required by the projection.</typeparam>
    /// <typeparam name="TResult">The type of data being projected.</typeparam>
    /// <param name="state">The caller's state, passed by reference so the projection can update it.</param>
    /// <param name="value">The reader, positioned on the element to project.</param>
    /// <returns>The projected value.</returns>
    ...
    /// <summary>
    /// Projects every child of the current aggregate into <paramref name="target"/>, which must be
    /// exactly the length reported by <see cref="AggregateLength"/>.
    /// </summary>
    ...
    /// <summary>
    /// Projects every child of the current aggregate into <paramref name="target"/>, which must be
    /// exactly the length reported by <see cref="AggregateLength"/>, passing caller state to the projection.
    /// </summary>
    /// <typeparam name="TState">Additional state required by the projection.</typeparam>
    ...
    /// <summary>
    /// Tries to read the current scalar element using a function-pointer parser; this is the
    /// allocation-free equivalent of the <see cref="ScalarParser{TSource, TValue}"/> overload.
    /// </summary>
    /// <typeparam name="T">The type of data being parsed.</typeparam>
    ...
    public unsafe bool TryReadNext() => TryReadNextCore();

    /// <summary>
    /// The implementation of <see cref="TryReadNext"/>, for the callers inside this library that
    /// genuinely are handling errors, attributes and streaming data themselves.
    /// </summary>
    /// <remarks>Upstream calls the obsolete public method directly and carries a
    /// "#pragma warning disable CS0618" at each site; this repository suppresses no warnings, so
    /// the body lives here and the obsolete member forwards to it. The behaviour is unchanged.</remarks>
    /// <returns><c>True</c> if an element was read, otherwise <c>False</c>.</returns>
    internal unsafe bool TryReadNextCore()
    {
        MovePastCurrent();
 ...
 Bmi1.TrailingZeroCount((uint)Avx.MoveMask(Unsafe.As<Vector256<uint>, Vector256<float>>(ref eqs)));
    ...
    /// <summary>
    /// Reads an aggregate whose children alternate between two kinds of value - a flattened map, for
    /// example - as an array of combined elements, moving past the data as a side effect.
    /// </summary>
    /// <typeparam name="TFirst">The type projected from the first element of each pair.</typeparam>
    /// <typeparam name="TSecond">The type projected from the second element of each pair.</typeparam>
    /// <typeparam name="TResult">The type of data to be projected.</typeparam>
    /// <param name="first">The projection applied to the first element of each pair.</param>
    /// <param name="second">The projection applied to the second element of each pair.</param>
    /// <param name="combine">Combines the two projected values into the result element.</param>
    /// <param name="scalar">When <c>true</c>, the children are known to be scalars, which allows a
    /// simpler traversal that does not account for RESP sub-trees.</param>
    /// <returns>The projected array, or <c>null</c> if the aggregate is null.</returns>
