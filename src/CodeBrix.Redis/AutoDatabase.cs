...
{
    // Upstream marks this member
    //     [Obsolete("Use MapInPlace instead; calling Map directly does nothing when the state is held by `in`.")]
    // purely as an in-repository trap, and then silences the resulting CS0618 with a pragma at the one
    // sanctioned call site (RedisArgsMutatorExtensions.MapInPlace below). The language offers no way to call
    // an obsolete member from non-obsolete code without a suppression, and this repository carries none, so
    // the attribute is recorded here as a comment instead. Nothing about the package changes: the interface
    // is internal, so no consumer ever saw the attribute, and the guard that actually matters - `ref this`
    // on MapInPlace, which turns an `in` state into CS8329 rather than a silently discarded copy - is
    // untouched. CALL THIS ONLY THROUGH MapInPlace.
