...
internal sealed partial class PhysicalConnection
{
        ...
        if (_output is { IsSync: true })
        {
            ...
            static void StartReadingSync(PhysicalConnection conn, CancellationToken cancellation)
            {
                ...
                Thread thread = new Thread(() => conn.ReadAllSync(cancellation))
                {
                    ...
                    Name = "CodeBrix.Redis Sync Reader",
                };
                thread.Start();
            }
        }
    ...
    private static void DebugValidateSingleFrame(ReadOnlySpan<byte> payload)
    {
        ...
        // Upstream calls the [Obsolete] RespReader.TryReadNext() here (its own comment: "we don't expect
        // *any* additional data, even attributes") and silences CS0618 with a pragma. Phase 1 moved that
        // member's body into the non-obsolete internal TryReadNextCore(), which the obsolete member now
        // forwards to, so this call site uses the core directly. Behaviour is identical.
        if (reader.TryReadNextCore())
        {
