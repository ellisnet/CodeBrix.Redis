...
using static CodeBrix.Redis.PhysicalBridge;
...
internal sealed partial class ServerEndPoint : IDisposable
{
    ...
    public WriteResult TryWriteSync(Message message) => TryWriteSyncCore(message);

    internal WriteResult TryWriteSyncCore(Message message) => GetBridge(message)?.TryWriteSyncCore(message, isReplica) ?? WriteResult.NoConnectionAvailable;
    ...
    internal bool CheckInfoReplication()
    {
            ...
            && GetBridge(ConnectionType.Interactive, false) is PhysicalBridge bridge)
        {
            ...
            bridge.TryWriteSyncCore(msg, isReplica);
            return true;
        }
        return false;
    }
