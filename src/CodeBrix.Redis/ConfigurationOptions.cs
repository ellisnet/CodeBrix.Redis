...
public sealed class ConfigurationOptions : ICloneable
{
    ...
    internal static LocalCertificateSelectionCallback CreatePemUserCertificateCallback(string userCertificatePath, string? userKeyPath)
    {
        ...
        // Upstream calls the X509Certificate2(byte[]) constructor here and silences SYSLIB0057 with a
        // pragma; this repository fixes it at source with the loader the obsoletion points at. The blob
        // is a PKCS#12 export from the line above, and LoadPkcs12 defaults to the same
        // X509KeyStorageFlags.DefaultKeySet the constructor used, so the result is identical.
        var pfx = X509CertificateLoader.LoadPkcs12(pem.Export(X509ContentType.Pfx), password: null);
    ...
    {
        // Upstream calls the X509Certificate2(string, string, X509KeyStorageFlags) constructor here and
        // silences SYSLIB0057 with a pragma; this repository fixes it at source. The file is a PFX (this
        // is the PFX overload, and a password is supplied), so LoadPkcs12FromFile is the equivalent
        // loader and takes the same password and storage flags.
        var pfx = X509CertificateLoader.LoadPkcs12FromFile(userCertificatePath, password ?? "", storageFlags);
        return (sender, targetHost, localCertificates, remoteCertificate, acceptableIssuers) => pfx;
    }
    ...
    // Upstream calls the X509Certificate2(string) constructor here and silences SYSLIB0057 with a
    // pragma; this repository fixes it at source. An issuer is a public certificate, and
    // LoadCertificateFromFile reads both the DER and the PEM encodings the constructor accepted for
    // one - verified against the upstream suite's own .pem issuer fixtures.
    internal static RemoteCertificateValidationCallback TrustIssuerCallback(string issuerCertificatePath)
        => TrustIssuerCallback(X509CertificateLoader.LoadCertificateFromFile(issuerCertificatePath));
    ...
    public SocketManager? SocketManager
    {
        ...
        set => socketManager = value;
    }

    // Upstream declares the property above as an auto-property and silences the CS0618 it causes in Clone()
    // and Reset() with pragmas. Here the backing store is a plain field, and it is typed object? rather than
    // SocketManager? so that those two non-obsolete members can copy and clear it without naming the obsolete
    // type. The public property, its type, its nullability and its [Obsolete] message are unchanged.
