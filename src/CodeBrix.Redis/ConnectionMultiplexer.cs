...
public sealed partial class ConnectionMultiplexer : IInternalConnectionMultiplexer // implies : IConnectionMultiplexer and : IDisposable
{
    ...
    private WriteResult TryPushMessageToBridgeSync<T>(Message message, ResultProcessor<T>? processor, IResultBox<T>? resultBox, [NotNullWhen(true)] ref ServerEndPoint? server)
        => TryPushMessageToBridgeSyncCore(message, processor, resultBox, ref server);

    private WriteResult TryPushMessageToBridgeSyncCore<T>(Message message, ResultProcessor<T>? processor, IResultBox<T>? resultBox, [NotNullWhen(true)] ref ServerEndPoint? server)
        => PrepareToPushMessageToBridge(message, processor, resultBox, ref server) ? server.TryWriteSyncCore(message) : WriteResult.NoConnectionAvailable;
    ...
    internal T? ExecuteSyncImpl<T>(Message message, ResultProcessor<T>? processor, ServerEndPoint? server, T? defaultValue = default)
    {
        ...
        {
            TryPushMessageToBridgeSyncCore(message, processor, null, ref server);
        ...
        else
        {
            ...
            lock (source)
            {
                result = TryPushMessageToBridgeSyncCore(message, processor, source, ref server);
