...
public class DefaultOptionsProvider
{
    ...
    protected virtual string GetDefaultClientName() =>
        (TryGetAzureRoleInstanceIdNoThrow()
         ?? ComputerName
         ?? "CodeBrix.Redis") + "(CodeBrix.Redis-v" + LibraryVersion + ")";

    /// <summary>
    /// Gets the library name to use for CLIENT SETINFO lib-name calls to Redis during handshake.
    /// Defaults to "CodeBrix.Redis".
    /// </summary>
    public virtual string LibraryName => "CodeBrix.Redis";

    /// <summary>
    /// String version of the CodeBrix.Redis library, for use in any options.
    /// </summary>
