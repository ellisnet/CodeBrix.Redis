...
namespace CodeBrix.Redis.Configuration; //was previously: StackExchange.Redis.Configuration;
...
public abstract class Tunnel
{
    ...
    /// <see cref="CodeBrix.Redis.Respite.Transports.DuplexTransport"/> the tunnel owns, and no socket is created at
    ...
    /// <see cref="CodeBrix.Redis.Respite.Transports.DuplexTransport.IsEncrypted"/>; if
    /// <see cref="TlsOptions.IsEnabled"/> is set and the transport reports otherwise, the
    /// connection is failed rather than used.
    /// </remarks>
    [Experimental(CodeBrix.Redis.Respite.Experiments.Transport, UrlFormat = CodeBrix.Redis.Respite.Experiments.UrlFormat)]
    public virtual ValueTask<CodeBrix.Redis.Respite.Transports.DuplexTransport?> ConnectTransportAsync(EndPoint endpoint, ConnectionType connectionType, TlsOptions tls, CancellationToken cancellationToken) => default;
