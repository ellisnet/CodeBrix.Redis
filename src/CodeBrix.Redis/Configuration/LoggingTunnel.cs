...
using CodeBrix.Redis.Respite.Transports;
using static CodeBrix.Redis.PhysicalConnection;
...
public abstract class LoggingTunnel : Tunnel
{
    ...
    protected sealed class LoggingDuplexTransport : DuplexTransport
    {
        ...
        /// <inheritdoc/>
        /// <remarks>The written bytes are copied into the outbound log before they are handed to the inner transport.</remarks>
    ...
    /// <summary>
    /// Captures the traffic of a duplex <see cref="Stream"/> into a pair of streams - one for the bytes
    /// read, one for the bytes written - in the format <see cref="LoggingTunnel"/> replays and validates.
    /// Seeking and length are not supported, because the underlying stream is a live connection.
    /// </summary>
