...
internal static class TestSetup
{
    ...
    /// CodeBrix.Redis build we reference gives CS1705 (assembly wants a newer System.Runtime), so
    ...
    /// </summary>
    /// <remarks>
    /// The analyzers here resolve CodeBrix.Redis types by metadata name and do nothing at all when they
    ...
    /// Hence both this and NoLibraryTests.cs, which asserts the absent case deliberately rather than by accident.
    /// </remarks>
    public static void Configure(AnalyzerTest<DefaultVerifier> test, bool referenceLibrary, string? minServerVersion)
    {
        ...
        if (referenceLibrary)
        {
            test.TestState.AdditionalReferences.Add(
                MetadataReference.CreateFromFile(typeof(CodeBrix.Redis.ConnectionMultiplexer).Assembly.Location));
        }
