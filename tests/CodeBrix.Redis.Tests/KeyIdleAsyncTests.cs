...
public class KeyIdleAsyncTests(ITestOutputHelper output, SharedConnectionFixture fixture) : TestBase(output, fixture)
{
    ...
    [Fact]
    public async Task idle_time_async()
    {
        ...
        (idleTime > TimeSpan.Zero).Should().BeTrue($"First check: {idleTime} should be > 0; elapsed: {timer.ElapsedMilliseconds}ms");

        db.StringSet(key, "new value2", flags: CommandFlags.FireAndForget);
        var idleTime2 = await db.KeyIdleTimeAsync(key).ForAwait();
        (idleTime2 < idleTime).Should().BeTrue($"Second check: {idleTime2} should be < {idleTime}; elapsed: {timer.ElapsedMilliseconds}ms");

        db.KeyDelete(key);
        var idleTime3 = await db.KeyIdleTimeAsync(key).ForAwait();
        idleTime3.Should().BeNull();
    }

    [Fact]
    public async Task touch_idle_time_async()
    {
        SkipOnWindowsRelease("WSL exacerbates replication time");
        ...
        (await db.KeyTouchAsync(key).ForAwait()).Should().BeTrue($"Second check: should be True; elapsed: {timer.ElapsedMilliseconds}ms");
        var idleTime1 = await db.KeyIdleTimeAsync(key).ForAwait();
        (idleTime1 < idleTime).Should().BeTrue($"Third check: {idleTime1} should be < {idleTime}; elapsed: {timer.ElapsedMilliseconds}ms");
    }
}
