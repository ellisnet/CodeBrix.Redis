...
public class Resp3HandshakeTests(ITestOutputHelper log)
{
    ...
    public async Task handshake(RedisProtocol client, ServerResponse server, HandshakeFlags flags)
    {
        ...
        clientObj.GetServerSnapshot()[0].Protocol.Should().Be(expectedProtocol);
        ...
        if (usePubSub)
        {
            ...
            Assert.Single(received).Should().Be("msg payload");
        }

        if (useDatabase)
        {
            (await db.StringGetAsync(key)).Should().Be("myvalue");
        }
    }
