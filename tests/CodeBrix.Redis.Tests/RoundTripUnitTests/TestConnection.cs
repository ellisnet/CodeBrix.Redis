...
using CodeBrix.Redis.Respite.Tests;
...
public class TestConnection : IDisposable
{
        ...
        [CallerMemberName] string caller = "")
    {
        ...
        string.IsNullOrEmpty(responseResp).Should().BeFalse("responseResp must not be null or empty");
        ...
        //the caller's token when it supplied one (a [Timeout] test hands over
        //TestContext.Current.CancellationToken), otherwise the ambient test token as before
        using var timeout = cancellationToken.CanBeCanceled ? tcs.CancelWith(cancellationToken) : tcs.CancelWithTest();

        message.SetSource(box, processor);
        conn.WriteOutbound(message, commandMap, channelPrefix);
        tcs.Task.Status.Should().Be(TaskStatus.WaitingForActivation); // should be pending, since we haven't responded yet
