...
public class HashImport(ITestOutputHelper log)
{
    ...
    [Fact(Timeout = 5000)]
    public async Task prepare_round_trips()
    {
        var token = CodeBrix.Redis.HashImport.Create("name", "email", "age");
    ...
    [Fact(Timeout = 5000)]
    public async Task set_round_trips()
    {
        var token = CodeBrix.Redis.HashImport.Create("f1", "f2");
        byte[] name = BitConverter.GetBytes(token.Id);
    ...
    [Fact(Timeout = 5000)]
    public async Task set_single_value_round_trips()
    {
        var token = CodeBrix.Redis.HashImport.Create("only");
        byte[] name = BitConverter.GetBytes(token.Id);
    ...
    [Fact(Timeout = 5000)]
    public async Task discard_round_trips()
    {
        var token = CodeBrix.Redis.HashImport.Create("f");
        byte[] name = BitConverter.GetBytes(token.Id);
