...
public class CommandRetryCategoryUnitTests(ITestOutputHelper log)
{
    ...
    [Fact]
    public async Task string_set_category_follows_condition()
    {
    ...
    [Fact]
    public async Task string_set_caller_category_wins()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
        RedisValue val = "v";
    ...
    [Fact]
    public async Task sort_store_is_a_write_not_a_read()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
    ...
    [Fact]
    public async Task sorted_set_add_increment_accumulates()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
        RedisValue member = "m";
    ...
    [Fact]
    public async Task stream_add_explicit_and_idempotent_ids_are_replay_safe()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
    ...
    /// </summary>
    [Fact]
    public async Task stream_add_partial_auto_id_still_accumulates()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
        var pair = new NameValueEntry("f", "v");
    ...
    /// </summary>
    [Fact]
    public async Task stream_read_group_claim_suppresses_the_demotion()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
    ...
    /// </summary>
    [Fact]
    public async Task expire_category_follows_condition()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
    ...
    /// </summary>
    [Fact]
    public async Task get_ex_ttl_options_make_it_a_write()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
    ...
    [Fact]
    public async Task copy_replace_is_an_unconditional_overwrite()
    {
        var db = await GetDatabaseAsync();
    ...
    [Fact]
    public async Task stream_claim_just_id_does_not_bump_delivery_counts()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
        RedisValue[] ids = ["5-5"];
    ...
    /// </summary>
    [Fact]
    public async Task geo_radius_typed_api_is_always_a_read()
    {
        var db = await GetDatabaseAsync();
        RedisKey key = "k";
    ...
    /// </summary>
    [Fact]
    public void script_load_is_connection_level_and_not_node_scoped()
    {
        ...
        ((msg.Flags & Message.CommandServerSpecific) != 0).Should().BeFalse("SCRIPT LOAD returns the same SHA from any node");
        ...
        Message.GetRetryCategory(fallback).Should().Be(CommandFlags.CommandRetryServerAdmin);
        ((fallback & Message.CommandServerSpecific) != 0).Should().BeTrue("bare SCRIPT stays node-scoped");
    }
    ...
    /// </summary>
    [Fact]
    public void server_sub_commands_are_categorized_by_sub_command()
    {
        ...
        ((purge.Flags & Message.CommandServerSpecific) != 0).Should().BeTrue("MEMORY PURGE is node-scoped");
        ...
        {
            ((msg.Flags & Message.CommandServerSpecific) != 0).Should().BeTrue($"{msg.CommandAndKey} should be node-scoped");
        }
        ...
        ((overridden.Flags & Message.CommandServerSpecific) != 0).Should().BeTrue("override must not clear server-specific");
    }

    [Fact]
    public void scan_cursor_only_resumed_cursors_are_server_specific()
    {
        // a fresh iteration can start on any node...
        var fresh = CommandFlags.None.WithScanCursorCategory(0);
        Message.GetRetryCategory(fresh).Should().Be(ReadOnly);
        ((fresh & Message.CommandServerSpecific) != 0).Should().BeFalse("cursor 0 should not be server-specific");
        ...
        Message.GetRetryCategory(resumed).Should().Be(ReadOnly);
        ((resumed & Message.CommandServerSpecific) != 0).Should().BeTrue("a resumed cursor should be server-specific");
        ...
        Message.GetRetryCategory(overridden).Should().Be(CallerOverride);
        ((overridden & Message.CommandServerSpecific) != 0).Should().BeTrue("caller category must not clear server-specific");
    }
}
