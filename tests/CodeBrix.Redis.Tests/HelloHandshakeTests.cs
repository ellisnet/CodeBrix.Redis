...
public class HelloHandshakeTests(ITestOutputHelper log)
{
    ...
    public async Task hello_is_issued_for_both_protocols(RedisProtocol protocol, string expectedProtover)
    {
        //Arrange
        ...
        //Assert
        hellos.Should().NotBeEmpty();
        hellos.Should().AllSatisfy(args => args.FirstOrDefault().Should().Be(expectedProtover));
        conn.GetServerSnapshot()[0].Protocol.Should().Be(protocol);
    }

    [Fact]
    public async Task no_hello_when_disabled_via_command_map()
    {
        //Arrange
        using var server = new RecordingServer(log);
        var config = server.GetClientConfig();
        ...
        //Assert
        server.Recorded("HELLO").Should().BeEmpty();
        conn.GetServerSnapshot()[0].Protocol.Should().Be(RedisProtocol.Resp2);
    }

    [Fact]
    public async Task no_hello_when_server_is_assumed_down_level()
    {
        using var server = new RecordingServer(log);
        var config = server.GetClientConfig();
        config.Protocol = RedisProtocol.Resp2;
    ...
    public async Task no_replica_probe_when_hello_tells_us_the_role(RedisProtocol protocol)
    {
        using var server = new RecordingServer(log);
        var config = server.GetClientConfig();
        config.Protocol = protocol;
        ...
        server.Recorded("HELLO").Should().NotBeEmpty();
        server.Recorded("SET").Should().BeEmpty();
        conn.GetServerSnapshot()[0].IsReplica.Should().BeFalse(); // from HELLO's "role"
    }
    ...
    /// </summary>
    [Fact]
    public async Task replica_probe_still_used_when_hello_unavailable()
    {
        using var server = new RecordingServer(log);
        var config = server.GetClientConfig();
        config.Protocol = RedisProtocol.Resp2;
        ...
        sets.Should().AllSatisfy(args => args.Skip(1).FirstOrDefault().Should().Be("replica-read-only"));
    }
    ...
    [InlineData(RedisProtocol.Resp2)]
    [InlineData(RedisProtocol.Resp3)]
    public async Task credentials_go_via_auth_not_hello(RedisProtocol protocol)
    {
        ...
        auths.Should().NotBeEmpty();
        auths.Should().AllSatisfy(args => args.FirstOrDefault().Should().Be("correcthorse"));

        // ...and HELLO is bare: just the protocol version, no AUTH (or SETNAME) clause
        var hellos = server.Recorded("HELLO");
        hellos.Should().NotBeEmpty();
        hellos.Should().AllSatisfy(args => Assert.Single(args));

        // and AUTH is issued first, so the connection is authenticated before HELLO runs
        server.RecordedOrder().First(x => x is "AUTH" or "HELLO").Should().Be("AUTH");
    }
    ...
    [Fact]
    public async Task hello_carries_credentials_when_auth_unavailable()
    {
        ...
        server.Recorded("AUTH").Should().BeEmpty();
        var hellos = server.Recorded("HELLO");
        hellos.Should().NotBeEmpty();
        hellos.Should().AllSatisfy(args => args.Skip(1).Take(3).Should().Equal(["AUTH", "default", "correcthorse"]));
        server.RecordedOrder().First().Should().Be("HELLO");
    }
