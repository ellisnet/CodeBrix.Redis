...
public class RedisValueStorageKindUnitTests(ITestOutputHelper output, InProcServerFixture fixture) : TestBase(output, fixture)
{
    ...
    public async Task fetched_value_storage_kind_and_round_trip(string stored, string expectedKind)
    {
        ...
        value.Type.ToString().Should().Be(expectedKind);
    }
    ...
    /// </summary>
    [Fact]
    public void literals_resolve_to_expected_storage_kinds()
    {
        ...
        byKind.Count.Should().Be(2);
        Count(RedisValue.StorageType.ByteArray).Should().Be(22); // literals > 8 bytes
    }
}
