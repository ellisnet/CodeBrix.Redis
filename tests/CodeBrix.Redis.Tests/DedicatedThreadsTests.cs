...
public class DedicatedThreadsTests(ITestOutputHelper output) : TestBase(output)
{
    ...
    [Fact]
    public Task without_the_flag_the_thread_pool_services_the_connection() => WithFlagAsync(false, (conn, endpoint) =>
    {
        conn.IsSyncReader(endpoint, ConnectionType.Interactive).Should().BeFalse();
        conn.IsSyncWriter(endpoint, ConnectionType.Interactive).Should().BeFalse();
        return Task.CompletedTask;
    });

    [Fact]
    public Task with_the_flag_the_interactive_connection_owns_its_threads() => WithFlagAsync(true, (conn, endpoint) =>
    {
        conn.IsSyncReader(endpoint, ConnectionType.Interactive).Should().BeTrue();
        conn.IsSyncWriter(endpoint, ConnectionType.Interactive).Should().BeTrue();
        return Task.CompletedTask;
    });
    ...
    /// </remarks>
    [Fact]
    public Task with_the_flag_pub_sub_stays_on_the_thread_pool() => WithFlagAsync(true, (conn, endpoint) =>
    {
        ...
        if (protocol >= RedisProtocol.Resp3)
        {
            ...
            conn.IsSyncReader(endpoint, ConnectionType.Subscription).Should().BeTrue();
            conn.IsSyncWriter(endpoint, ConnectionType.Subscription).Should().BeTrue();
        }
        else
        {
            conn.IsSyncReader(endpoint, ConnectionType.Subscription).Should().BeFalse();
            conn.IsSyncWriter(endpoint, ConnectionType.Subscription).Should().BeFalse();
