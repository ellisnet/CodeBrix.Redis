...
public static class Skip
{
    ...
    /// <summary>
    /// Skips the test when the container-backed tier is off.
    /// </summary>
    /// <remarks>
    /// NEW in this repository. Every test that needs a real Redis server needs the harness's
    /// containers, and they only run when <c>CODEBRIX_REDIS_RUN_CONTAINER_TESTS=1</c> is set. This
    /// is the one place that decision is expressed; <see cref="IfNoServer"/> calls it first, so the
    /// existing <c>Skip.IfNoCluster()</c> / <c>IfNoSentinel()</c> / <c>IfNoFailoverPair()</c> call
    /// sites - and any test that probes an endpoint before connecting - inherit it unchanged.
    /// </remarks>
    public static void IfNoContainers()
    {
        Assert.SkipWhen(!TestConfig.ServersAvailable, TestConfig.NoServersReason);
    }
    ...
    {
        //Ask the tier gate before touching the network: with the gate off there is deliberately
        //nothing to connect to, and a probe would be a wasted syscall reported as the wrong reason.
        IfNoContainers();
    ...
    /// <summary>
    /// Skips the test when the TLS server is not running, and returns the authority certificate
    /// the harness generated for this run.
    /// </summary>
    /// <remarks>
    /// NEW in this repository. Upstream validated the TLS endpoint against a checked-in server
    /// certificate; that certificate expired in 2023 and its authority key was never in the
    /// repository, so the harness generates a fresh authority per run instead. A test puts the
    /// returned certificate in its <c>X509Chain</c>'s <c>ExtraStore</c>.
    /// </remarks>
    /// <returns>The run's certificate authority.</returns>
    public static System.Security.Cryptography.X509Certificates.X509Certificate2 IfNoTlsServer()
    {
        var config = TestConfig.Current;
        IfNoServer(config.SslServer, config.SslPort);
        var authority = RedisTopologyFixture.TlsCaCertificate;
        Assert.SkipWhen(
            authority is null,
            "The TLS topology did not publish a certificate authority for this run.");
        return authority!;
    }
