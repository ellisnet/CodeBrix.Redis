...
[assembly: AssemblyFixture(typeof(CodeBrix.Redis.Tests.SharedConnectionFixture))]

namespace CodeBrix.Redis.Tests; //was previously: StackExchange.Redis.Tests;

public class SharedConnectionFixture : IDisposable
{
    //The shared connection is only useful when there is a server to share; with the container
    //tier off there is not, and every test that would have used it skips through Skip.* first.
    public bool IsEnabled { get; } = TestConfig.Current.UseSharedConnection && TestConfig.ServersAvailable;
