...
{
    //was previously: an override of the five-argument CreateTestCasesForDataRow. xunit.v3 4.0 added a
    //sixth parameter (the optional zero-padded display-name index) and made THAT overload the virtual
    //one; the five-argument form is now non-virtual and forwards to it.
    protected override ValueTask<IReadOnlyCollection<IXunitTestCase>> CreateTestCasesForDataRow(ITestFrameworkDiscoveryOptions discoveryOptions, IXunitTestMethod testMethod, ITheoryAttribute theoryAttribute, ITheoryDataRow dataRow, object?[] testMethodArguments, string? index)
        => base.CreateTestCasesForDataRow(discoveryOptions, testMethod, theoryAttribute, dataRow, testMethodArguments, index).ExpandAsync();
...
public class ProtocolTestCase : XunitTestCase, IProtocolTestCase
{
    ...
    public ProtocolTestCase(XunitTestCase testCase, RedisProtocol protocol) : base(
        ...
        testCaseDisplayName: $"{testCase.TestCaseDisplayName.Replace("CodeBrix.Redis.Tests.", "")} ({protocol.GetString()})",
