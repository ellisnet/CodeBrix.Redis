...
public class UnknownPushUnitTests(ITestOutputHelper log)
{
    ...
    [Fact]
    public async Task unknown_push_is_ignored_while_command_outstanding()
    {
        ...
        server.Injected.Should().Be(1);
        value.Should().Be("abc");
    }

    [Fact]
    public async Task connection_survives_unknown_push_and_keeps_working()
    {
        using var server = new UnknownPushServer(log);
        await using var conn = await server.ConnectResp3Async();
