...
public class ReconnectRetryPolicyUnitTests(ITestOutputHelper log)
{
    ...
    public async Task retry_policy_failure_cases(FailureMode failureMode)
    {
        ...
        else
        {
            ...
            (actual is "0,1" or "0,1,2").Should().BeTrue($"unexpected counts: {actual}");
        }
    }
    ...
    private sealed class NonResponsiveServer(ITestOutputHelper log) : InProcessTestServer(log)
    {
        ...
        protected override ValueTask OnAcceptClientAsync(EndPoint endpoint)
        {
            ...
            {
                await Task.Delay(TimeSpan.FromMinutes(5), TestContext.Current.CancellationToken).ConfigureAwait(false);
