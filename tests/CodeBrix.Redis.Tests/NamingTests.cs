...
public class NamingTests(ITestOutputHelper output) : TestBase(output)
{
    ...
    public void check_signatures(Type type, bool isAsync)
    {
    ...
    /// </summary>
    [Fact]
    public void check_read_only_operations()
    {
    ...
    public void check_database_methods_use_keys(Type type)
    {
        foreach (var method in type.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly))
        {
            ...
            usesKey.Should().BeTrue(type.Name + ":" + method.Name);
        }
    }
    ...
    public void check_sync_async_methods_match(Type from, Type to)
    {
        ...
        foreach (var method in from.GetMethods(flags))
        {
            ...
            {
                args.Last().Should().Be(typeof(CommandFlags));
            }
            var found = to.GetMethod(huntName, flags, null, method.CallingConvention, args, null);
            //kept as the xUnit form: it carries [NotNull], so the compiler's null-state flows to the dereference below
            Assert.NotNull(found); // "Found " + name + ", no " + huntName
            var pTo = found.GetParameters();

            for (int i = 0; i < pFrom.Length; i++)
            {
                pTo[i].Name.Should().Be(pFrom[i].Name); // method.Name + ":" + pFrom[i].Name
                pTo[i].ParameterType.Should().Be(pFrom[i].ParameterType); // method.Name + ":" + pFrom[i].Name
            }
    ...
    private void CheckMethod(MethodInfo method, bool isAsync)
    {
        ...
        switch (shortName)
        {
            ...
            default:
                ...
                isValid.Should().BeTrue(fullName + ":Prefix");
                break;
        }

        shortName.Contains("If").Should().BeFalse(fullName + ":If"); // should probably be a When option

        var returnType = method.ReturnType ?? typeof(void);

        if (isAsync)
        {
            IsAsyncMethod(returnType).Should().BeTrue(fullName + ":Task");
        }
        else
        {
            IsAsyncMethod(returnType).Should().BeFalse(fullName + ":Task");
        }
    ...
    {
        if (isAsync) member.Name.EndsWith("Async").Should().BeTrue(member.Name + ":Name - end *Async");
        else member.Name.EndsWith("Async").Should().BeFalse(member.Name + ":Name - don't end *Async");
    }
}
