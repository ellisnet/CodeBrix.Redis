using System.Linq;
using SilverAssertions;
using Xunit;

namespace CodeBrix.Redis.Tests.ResultProcessorUnitTests; //was previously: StackExchange.Redis.Tests.ResultProcessorUnitTests;

public class LongestCommonSubsequence(ITestOutputHelper log) : ResultProcessorUnitTest(log)
{
    [Fact]
    public void single_match_success()
    {
        // LCS key1 key2 IDX MINMATCHLEN 4 WITHMATCHLEN
        // 1) "matches"
        // 2) 1) 1) 1) (integer) 4
        //          2) (integer) 7
        //       2) 1) (integer) 5
        //          2) (integer) 8
        //       3) (integer) 4
        // 3) "len"
        // 4) (integer) 6
        var resp = "*4\r\n$7\r\nmatches\r\n*1\r\n*3\r\n*2\r\n:4\r\n:7\r\n*2\r\n:5\r\n:8\r\n:4\r\n$3\r\nlen\r\n:6\r\n";
        var result = Execute(resp, ResultProcessor.LCSMatchResult);

        result.LongestMatchLength.Should().Be(6);
        result.Matches.Should().ContainSingle();

        // Verify backward-compatible properties
        result.Matches[0].FirstStringIndex.Should().Be(4);
        result.Matches[0].SecondStringIndex.Should().Be(5);
        result.Matches[0].Length.Should().Be(4);

        // Verify new Position properties
        result.Matches[0].First.Start.Should().Be(4);
        result.Matches[0].First.End.Should().Be(7);
        result.Matches[0].Second.Start.Should().Be(5);
        result.Matches[0].Second.End.Should().Be(8);
    }

    [Fact]
    public void two_matches_success()
    {
        // LCS key1 key2 IDX MINMATCHLEN 0 WITHMATCHLEN
        // 1) "matches"
        // 2) 1) 1) 1) (integer) 4
        //          2) (integer) 7
        //       2) 1) (integer) 5
        //          2) (integer) 8
        //       3) (integer) 4
        //    2) 1) 1) (integer) 2
        //          2) (integer) 3
        //       2) 1) (integer) 0
        //          2) (integer) 1
        //       3) (integer) 2
        // 3) "len"
        // 4) (integer) 6
        var resp = "*4\r\n$7\r\nmatches\r\n*2\r\n*3\r\n*2\r\n:4\r\n:7\r\n*2\r\n:5\r\n:8\r\n:4\r\n*3\r\n*2\r\n:2\r\n:3\r\n*2\r\n:0\r\n:1\r\n:2\r\n$3\r\nlen\r\n:6\r\n";
        var result = Execute(resp, ResultProcessor.LCSMatchResult);

        result.LongestMatchLength.Should().Be(6);
        result.Matches.Length.Should().Be(2);

        // First match - verify backward-compatible properties
        result.Matches[0].FirstStringIndex.Should().Be(4);
        result.Matches[0].SecondStringIndex.Should().Be(5);
        result.Matches[0].Length.Should().Be(4);

        // First match - verify new Position properties
        result.Matches[0].First.Start.Should().Be(4);
        result.Matches[0].First.End.Should().Be(7);
        result.Matches[0].Second.Start.Should().Be(5);
        result.Matches[0].Second.End.Should().Be(8);

        // Second match - verify backward-compatible properties
        result.Matches[1].FirstStringIndex.Should().Be(2);
        result.Matches[1].SecondStringIndex.Should().Be(0);
        result.Matches[1].Length.Should().Be(2);

        // Second match - verify new Position properties
        result.Matches[1].First.Start.Should().Be(2);
        result.Matches[1].First.End.Should().Be(3);
        result.Matches[1].Second.Start.Should().Be(0);
        result.Matches[1].Second.End.Should().Be(1);
    }

    [Fact]
    public void no_matches_success()
    {
        // LCS key1 key2 IDX
        // 1) "matches"
        // 2) (empty array)
        // 3) "len"
        // 4) (integer) 0
        var resp = "*4\r\n$7\r\nmatches\r\n*0\r\n$3\r\nlen\r\n:0\r\n";
        var result = Execute(resp, ResultProcessor.LCSMatchResult);

        result.LongestMatchLength.Should().Be(0);
        result.Matches.Should().BeEmpty();
    }

    [Fact]
    public void not_array_failure()
    {
        var resp = "+OK\r\n";
        ExecuteUnexpected(resp, ResultProcessor.LCSMatchResult);
    }
}
