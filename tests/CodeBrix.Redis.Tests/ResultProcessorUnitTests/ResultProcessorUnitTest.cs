...
public abstract class ResultProcessorUnitTest(ITestOutputHelper log)
{
    ...
    {
        TryExecute(resp, processor, out _, out var ex, message, connectionType, protocol, caller).Should().BeFalse(caller);
        if (ex is not null) Log(ex.Message);
        (Assert.IsType<RedisConnectionException>(ex).Message).Should().StartWith("Unexpected response to UNKNOWN:");
    }
    private protected static T? Execute<T>(
        ...
        [CallerMemberName] string caller = "")
    {
        (TryExecute<T>(resp, processor, out var value, out var ex, message, connectionType, protocol, caller)).Should().BeTrue();
        ex.Should().BeNull();
        return value;
    }
        ...
        [CallerMemberName] string caller = "")
    {
        ...
        try
        {
            ...
            processor.SetResult(connection, message, ref reader).Should().BeTrue();
