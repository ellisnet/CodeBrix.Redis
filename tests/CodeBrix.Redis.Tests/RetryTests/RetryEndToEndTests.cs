...
public class RetryEndToEndTests(ITestOutputHelper log) : TestBase(log)
{
    ...
    // one succeeded.
    [Fact]
    public async Task with_retry_rides_out_transient_loading()
    {
        ...
        (await db.StringSetAsync(key, "hello")).Should().BeTrue(); // seed the value before we start failing GETs
        ...
        value.Should().Be("hello"); // retries rode out the LOADING responses
        server.LoadingOps.Should().Be(0); // both LOADING responses were consumed
        server.GetOpsReceived.Should().Be(3); // 2 x LOADING + 1 x success
    }
    ...
    [Fact]
    public async Task with_retry_when_attempts_exhausted_throws_original_fault()
    {
        using var server = new LoadingServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        (await db.StringSetAsync(key, "hello")).Should().BeTrue();
        ...
        ex.Kind.Should().Be(RedisErrorKind.Loading);
        server.GetOpsReceived.Should().Be(3); // tried exactly MaxAttempts times
    }
    ...
    [Fact]
    public async Task with_retry_non_transient_fault_is_not_retried()
    {
        using var server = new LoadingServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        ex.Kind.Should().Be(RedisErrorKind.WrongType);
        server.GetOpsReceived.Should().Be(1); // gave up immediately
    }
    ...
    [Fact]
    public async Task with_retry_ad_hoc_command_inherits_known_command_category()
    {
        using var server = new LoadingServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        server.GetOpsReceived.Should().Be(3); // recognised as GET, i.e. read-only, so retried
    }
    ...
    public async Task with_retry_unrecognised_command_respects_supplied_category(CommandFlags flags, int expectedAttempts)
    {
        using var server = new LoadingServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        else
        {
            ...
            result.AsString().Should().Be("made-up-ok");
        }

        server.UnknownOpsReceived.Should().Be(expectedAttempts);
    }
    ...
    [Fact]
    public async Task with_retry_fails_over_between_groups_on_loading()
    {
        ...
        {
            (await seedA.GetDatabase().StringSetAsync(key, "from-A")).Should().BeTrue();
        }
        await using (var seedB = await serverB.ConnectAsync())
        {
            (await seedB.GetDatabase().StringSetAsync(key, "from-B")).Should().BeTrue();
        }
        ...
        conn.IsConnected.Should().BeTrue();
        conn.ActiveMember.Should().BeSameAs(members[0]); // A is active (highest weight)
        ...
        after.Should().Be("from-B");
        conn.ActiveMember.Should().BeSameAs(members[1]); // we really did move to B
    }
    ...
    [Fact]
    public async Task with_retry_when_failover_never_arrives_keeps_retrying_same_member()
    {
        EndPoint alpha = new DnsEndPoint("alpha", 6379);
        EndPoint bravo = new DnsEndPoint("bravo", 6379);
        ...
        await using var conn = await ConnectionMultiplexer.ConnectGroupAsync(members, options);
        conn.ActiveMember.Should().BeSameAs(members[0]);
        ...
        (await db.StringGetAsync(key)).Should().Be("from-A");
        serverA.GetOpsReceived.Should().Be(3); // 2 x LOADING + 1 x success, all on A
        conn.ActiveMember.Should().BeSameAs(members[0]); // never moved to B
    }
    ...
    [Fact]
    public async Task with_retry_transaction_rides_out_transient_exec()
    {
        ...
        (await db.StringSetAsync(key, "seed")).Should().BeTrue(); // seed a value we expect the transaction to overwrite
        ...
        committed.Should().BeTrue(); // rode out the transient EXEC failure
        (await setTask).Should().BeTrue(); // per-op proxy resolved from the winning attempt
        (await getTask).Should().Be("committed"); // read reflects the committed value
        (await db.StringGetAsync(key)).Should().Be("committed"); // and it really landed on the server
        server.FailExecOps.Should().Be(0); // the transient failure was consumed
        server.ExecOpsReceived.Should().Be(2); // 1 x LOADING + 1 x commit
    }
    ...
    [Fact]
    public async Task with_retry_transaction_rejected_exec_retries_regardless_of_category()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        conn.IsConnected.Should().BeTrue();
        ...
        RetryPolicy policy = new RetryPolicy.Builder { MaxAttempts = 3, RetryDelay = TimeSpan.Zero, JitterMax = TimeSpan.Zero };
        policy.MaxCommandRetryCategory.Should().Be(CommandFlags.CommandRetryWriteLastWins); // i.e. the default
        ...
        (await incr).Should().Be(1); // the discarded attempt applied nothing -> incremented exactly once
        server.ExecOpsReceived.Should().Be(2); // 1 x LOADING + 1 x commit
    }
    ...
    public async Task with_retry_ambiguous_fault_is_still_gated_by_category(bool allowAccumulating, int expectedValue)
    {
        ...
        ex.Kind.Should().Be(RedisErrorKind.OutOfMemory);

        server.IncrOpsReceived.Should().Be(expectedValue);
        server.FailIncr = false;
        ((long)await db.StringGetAsync(key)).Should().Be(expectedValue); // and each one really did apply
    }
    ...
    [Fact]
    public async Task with_retry_transaction_satisfied_condition_rides_out_transient_exec()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        conn.IsConnected.Should().BeTrue();
        ...
        committed.Should().BeTrue(); // rode out the transient EXEC failure
        cond.WasSatisfied.Should().BeTrue(); // durable condition result forwarded from the winning attempt
        (await setTask).Should().BeTrue(); // per-op proxy resolved
        (await db.StringGetAsync(key)).Should().Be("committed"); // the write really landed
        server.ExecOpsReceived.Should().Be(2); // 1 x LOADING + 1 x commit -> the WATCH replayed
    }
    ...
    [Fact]
    public async Task with_retry_transaction_unsatisfied_condition_aborts_without_retry()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        conn.IsConnected.Should().BeTrue();
        ...
        committed.Should().BeFalse(); // electively aborted via the failed WATCH
        cond.WasSatisfied.Should().BeFalse();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(async () => await setTask); // proxy cancelled, not hung
        (await db.StringGetAsync(key)).Should().Be("seed"); // nothing was written
        server.ExecOpsReceived.Should().Be(0); // never EXEC'd, and (the point) never retried
    }
    ...
    [Fact]
    public async Task with_retry_transaction_per_op_error_faults_only_that_proxy()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        conn.IsConnected.Should().BeTrue();
        ...
        (await db.StringSetAsync(badKey, "abc")).Should().BeTrue(); // non-numeric: INCR will error at EXEC time
        ...
        committed.Should().BeTrue(); // EXEC still committed
        (await goodTask).Should().BeTrue(); // the good op resolved
        await Assert.ThrowsAsync<RedisServerException>(async () => await badTask); // only the bad op faulted
        (await db.StringGetAsync(goodKey)).Should().Be("ok"); // the good write landed
        server.ExecOpsReceived.Should().Be(1); // committed first time; no retry
    }
    ...
    // with_retry_fails_over_between_groups_on_loading test.
    [Fact]
    public async Task with_retry_transaction_fails_over_between_groups()
    {
        EndPoint alpha = new DnsEndPoint("alpha", 6379);
        EndPoint bravo = new DnsEndPoint("bravo", 6379);
        ...
        committed.Should().BeTrue(); // rode the failover across to B and committed there
        (await setTask).Should().BeTrue();
        conn.ActiveMember.Should().BeSameAs(members[1]); // we really did move to B

        // the write landed on B, not A
        await using var checkB = await serverB.ConnectAsync();
        (await checkB.GetDatabase().StringGetAsync(key)).Should().Be("committed-on-B");
    }
    ...
    [Fact]
    public async Task with_retry_transaction_replays_repeatedly()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        (await incr).Should().Be(1); // the two discarded attempts applied nothing
        (await push).Should().Be(1);
        (await get).Should().Be("committed");
        server.ExecOpsReceived.Should().Be(3); // 2 x LOADING + 1 x commit
    }
    ...
    [Fact]
    public async Task with_retry_transaction_when_attempts_exhausted_faults_every_proxy()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        (await Assert.ThrowsAsync<RedisServerException>(async () => await set)).Should().BeSameAs(fault);
        (await Assert.ThrowsAsync<RedisServerException>(async () => await get)).Should().BeSameAs(fault);
        server.ExecOpsReceived.Should().Be(2); // exactly MaxAttempts
    }
    ...
    [Fact]
    public async Task with_retry_transaction_watch_drift_is_reattempted()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        conn.IsConnected.Should().BeTrue();
        ...
        if (await Task.WhenAny(execute, Task.Delay(5000, TestContext.Current.CancellationToken)) != execute)
        {
            Assert.Fail("ExecuteAsync never completed");
        }

        (await execute).Should().BeTrue(); // rode out the lost race
        cond.WasSatisfied.Should().BeTrue();
        (await setTask).Should().BeTrue();
        tran.WasWatchConflict.Should().BeFalse(); // reports the *final* attempt: we got there in the end
        server.ExecOpsReceived.Should().Be(2); // 1 x watch conflict + 1 x commit
        (await db.StringGetAsync(key)).Should().Be("committed");
    }
    ...
    [Fact]
    public async Task with_retry_transaction_watch_drift_can_be_disabled()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        tran.WasWatchConflict.Should().BeTrue(); // ...and the caller can see exactly that
    ...
    [Fact]
    public async Task with_retry_transaction_persistent_watch_drift_gives_up()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        cond.WasSatisfied.Should().BeTrue();
        tran.WasWatchConflict.Should().BeTrue(); // still losing the race when we ran out of attempts
        server.ExecOpsReceived.Should().Be(4); // bounded by MaxAttemptsOnWatchConflict
        await Assert.ThrowsAnyAsync<OperationCanceledException>(async () => await setTask);
    ...
    [Fact]
    public async Task with_retry_transaction_without_conditions_ignores_watch_budget()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
    ...
    [Fact]
    public async Task with_retry_transaction_is_an_i_transaction()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        tran.Execute().Should().BeTrue(); // sync-over-async; note this binds to ITransaction's bool overload

        (await set).Should().BeTrue(); // the durable proxies resolve exactly as on the async path
        (await get).Should().Be("committed");
        (await db.StringGetAsync(key)).Should().Be("committed");
        server.ExecOpsReceived.Should().Be(1);
    }
    ...
    [Fact]
    public async Task with_retry_transaction_sync_execute_rides_out_transient_exec()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        cond.WasSatisfied.Should().BeTrue(); // the WATCH constraint was replayed onto the winning attempt
        (await set).Should().BeTrue();
        (await db.StringGetAsync(key)).Should().Be("committed");
        server.ExecOpsReceived.Should().Be(2); // 1 x LOADING + 1 x commit, all within Execute()
    }
    ...
    [Fact]
    public async Task with_retry_transaction_sync_execute_unsatisfied_condition_returns_false()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        tran.Execute().Should().BeFalse();

        cond.WasSatisfied.Should().BeFalse();
        tran.WasWatchConflict.Should().BeFalse(); // an elective abort, not a lost race
        await Assert.ThrowsAnyAsync<OperationCanceledException>(async () => await set);
        (await db.StringGetAsync(key)).Should().Be("seed");
        server.ExecOpsReceived.Should().Be(0); // never EXEC'd
    }
    ...
    [Fact]
    public async Task with_retry_transaction_sync_execute_when_attempts_exhausted_throws_original_fault()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        (await Assert.ThrowsAsync<RedisServerException>(async () => await set)).Should().BeSameAs(fault); // proxy faulted too
        server.ExecOpsReceived.Should().Be(2); // exactly MaxAttempts
    }
    ...
    [Fact]
    public async Task with_retry_transaction_batch_execute_is_fire_and_forget()
    {
        using var server = new ExecFailServer(Output);
        await using var conn = await server.ConnectAsync(log: Writer);
        ...
        (await set).Should().BeTrue(); // ...but the per-operation proxy still resolves
        (await db.StringGetAsync(key)).Should().Be("committed");
        server.ExecOpsReceived.Should().Be(1);
    }
    ...
    [Fact]
    public async Task with_retry_transaction_batch_execute_does_not_wait_for_replies()
    {
