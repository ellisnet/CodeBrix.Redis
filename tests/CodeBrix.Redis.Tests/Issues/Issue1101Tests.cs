...
public class Issue1101Tests(ITestOutputHelper output) : TestBase(output)
{
    private static void AssertCounts(ISubscriber pubsub, in RedisChannel channel, bool has, int handlers, int queues)
    {
        if (pubsub.Multiplexer is ConnectionMultiplexer muxer)
        {
            ...
            aHas.Should().Be(has);
            ah.Should().Be(handlers);
            aq.Should().Be(queues);
        }
    }

    [Fact]
    public async Task execute_with_unsubscribe_via_channel()
    {
        ...
        lock (values)
        {
            Assert.Single(values).Should().Be("abc");
        }
        var subs = conn.GetServer(conn.GetEndPoints().Single()).SubscriptionSubscriberCount(name);
        subs.Should().Be(1);
        first.Completion.IsCompleted.Should().BeFalse("completed");
        second.Completion.IsCompleted.Should().BeFalse("completed");
        ...
        Volatile.Read(ref i).Should().Be(2);
        first.Completion.IsCompleted.Should().BeTrue("completed");
        second.Completion.IsCompleted.Should().BeFalse("completed");
        ...
        Volatile.Read(ref i).Should().Be(2);
        first.Completion.IsCompleted.Should().BeTrue("completed");
        second.Completion.IsCompleted.Should().BeTrue("completed");
        AssertCounts(pubsub, name, false, 0, 0);
    ...
    [Fact]
    public async Task execute_with_unsubscribe_via_subscriber()
    {
        ...
        Volatile.Read(ref i).Should().Be(1);
    ...
    [Fact]
    public async Task execute_with_unsubscribe_via_clear_all()
    {
        await using var conn = Create(log: Writer);
