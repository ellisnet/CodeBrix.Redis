...
public class BasicMultiGroupTests(ITestOutputHelper log)
{
    private sealed class Capture(ITestOutputHelper log)
    {
        ...
        public void AssertTakeAny(string value)
        {
            lock (_seen)
            {
                _seen.Remove(value).Should().BeTrue($"Expected to find '{value}', but did not");
            }
        }
        ...
        public void AssertTakeFirst(string value)
        {
            lock (_seen)
            {
                _seen.Should().NotBeEmpty();
                _seen[0].Should().Be(value);
        ...
        public async ValueTask AwaitAsync(ISubscriber sub, int expected)
        {
            for (int i = 0; i < 10; i++)
            {
                lock (_seen)
                {
                    ...
                    {
                        _seen.Count.Should().Be(expected);
    ...
    public async Task select_by_weight(InbuiltProbe probe, ServerType serverType)
    {
    ...
    [Fact]
    public async Task select_by_latency()
    {
        EndPoint germany = new DnsEndPoint("germany", 6379);
        EndPoint canada = new DnsEndPoint("canada", 6379);
        EndPoint tokyo = new DnsEndPoint("tokyo", 6379);
        ...
        (await SelectPreferredAsync(typed, db, canada)).Should().Be(canada);

        // change latency and update
        server0.SetLatency(TimeSpan.FromMilliseconds(150));
        server1.SetLatency(TimeSpan.FromMilliseconds(150));
        server2.SetLatency(TimeSpan.Zero);
        (await SelectPreferredAsync(typed, db, tokyo)).Should().Be(tokyo);
    }

    [Fact]
    public async Task pub_sub_routed()
    {
        EndPoint germany = new DnsEndPoint("germany", 6379);
        EndPoint canada = new DnsEndPoint("canada", 6379);
        EndPoint tokyo = new DnsEndPoint("tokyo", 6379);
    ...
    [Fact]
    public async Task pub_sub_ordered_routed()
    {
        EndPoint germany = new DnsEndPoint("germany", 6379);
        EndPoint canada = new DnsEndPoint("canada", 6379);
        EndPoint tokyo = new DnsEndPoint("tokyo", 6379);
