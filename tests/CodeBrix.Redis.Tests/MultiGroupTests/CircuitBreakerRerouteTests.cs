...
public class CircuitBreakerRerouteTests(ITestOutputHelper log) : TestBase(log)
{
    ...
    [Fact]
    public async Task circuit_breaker_trip_reroutes_away_from_member()
    {
        ...
        moved.Should().BeTrue("expected the circuit-breaker trip to reroute away from member A");
        conn.ActiveMember.Should().BeSameAs(members[1]); // B is the next-highest weight
        ...
        (await circuitBreakerEvents.Task.WaitAsync(cbCts.Token)).Should().BeTrue("expected a ConnectionFailed event with FailureType == CircuitBreaker");
    }
