...
{
    [Fact]
    public async Task parallel_transactions_with_conditions()
    {
        ...
        try
        {
            ...
            actual.Should().Be(expectedSuccess);
    ...
    [Fact]
    public async Task run_competing_batches_on_same_muxer()
    {
        Skip.UnlessLongRunning();
    ...
    [Fact]
    public async Task run_competing_batches_on_same_muxer_async()
    {
        Skip.UnlessLongRunning();
        await using var conn = Create();
        var db = conn.GetDatabase();

        var x = Task.Run(() => BatchRunPingsAsync(db), TestContext.Current.CancellationToken);
        var y = Task.Run(() => BatchRunIntegersAsync(db), TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task run_competing_transactions_on_same_muxer()
    {
        Skip.UnlessLongRunning();
    ...
    [Fact]
    public async Task run_competing_transactions_on_same_muxer_async()
    {
        Skip.UnlessLongRunning();
        await using var conn = Create(logTransactionData: false);
        var db = conn.GetDatabase();

        var x = Task.Run(() => TranRunPingsAsync(db), TestContext.Current.CancellationToken);
        var y = Task.Run(() => TranRunIntegersAsync(db), TestContext.Current.CancellationToken);
