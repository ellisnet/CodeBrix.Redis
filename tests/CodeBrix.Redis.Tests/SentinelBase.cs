...
public class SentinelBase : TestBase, IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        {
            if (i != 0) await Task.Delay(ConnectRetryDelay, TestContext.Current.CancellationToken).ForAwait(); // check first, then back off
        ...
        primaryConnected.Should().BeTrue(diagnostics);
    ...
    protected async Task WaitForReplicationAsync(IServer primary, TimeSpan? duration = null)
    {
        ...
        while (sw.Elapsed < duration.Value)
        {
            ...
            await Task.Delay(250, TestContext.Current.CancellationToken).ForAwait();
        }
