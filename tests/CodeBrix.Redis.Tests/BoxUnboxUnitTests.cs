...
public class BoxUnboxUnitTests
{
    ...
    public void round_trip_redis_value(RedisValue value)
    {
        var boxed = value.Box();
    ...
    public void unbox_common_values(object value, RedisValue expected)
    {
    ...
    public void return_interned_boxes_for_common_values(RedisValue value, bool expectSameReference, string label)
    {
        _ = label;
        object? x = value.Box(), y = value.Box();
        ReferenceEquals(x, y).Should().Be(expectSameReference);
